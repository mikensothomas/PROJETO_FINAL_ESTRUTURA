...
No *inserir(No *raiz, Jogador jogador, char chaveDeBusca[]) {
    if (raiz == NULL) {
...
No *buscar(No *raiz, int pontos, char chaveDeBusca[]) {
...
No *remover(No *raiz, int pontos, char chaveDeBusca[]) {
    if (raiz == NULL) {
        return raiz;
    }
...
void listarEmOrdem(No *raiz, char chaveDeBusca[]) {
    if (raiz != NULL) {
...
int main() {
    ...
    No *resultadoBusca = buscar(arvore, 15, "pontos");
